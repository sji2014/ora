defmodule Ora.PayNow do
  ...
  def pad_left(str, n, pad_str \\ "0") do
    if String.length(str) >= n do
      ...
      String.duplicate(pad_str, n - String.length(str)) <> str
    end
  end

  def gen_qr(opts) do
    image = {:code.priv_dir(:ora) |> to_string() |> Kernel.<>("/static/images/signum.svg"), 100}

    settings = %QRCode.Render.SvgSettings{
      qrcode_color: {0, 104, 55},
      image: image,
      scale: 5,
      structure: :minify
    }
    ...
    p = [
      ...
      %{
        ...
        value: [
          ...
          %{id: "02", value: opts[:uen]},
          # // 1 = Payment amount is editable, 0 = Not Editable
          %{id: "03", value: Integer.to_string(opts[:editable])},
      ...
      %{
        id: "54",
        value: Float.to_string(opts[:amount]) |> then(&Regex.replace(~r/\.?0+$/, &1, "\\1"))
      },
      # // ID 58: 2-letter Country Code (SG)
      %{id: "58", value: "SG"},
      # ID 59: Company Name
      %{id: "59", value: opts[:company]},
      # // ID 60: Merchant City
      ...
      %{
        ...
        value: [
          # / ID 01: Reference ID
          %{id: "01", value: opts[:ref_number]}
        ]
      }
    ]

    str =
      Enum.reduce(p, "", fn current, final ->
        value =
          if is_list(current.value) do
            Enum.reduce(current.value, "", fn c, f ->
              f <> c.id <> pad_left(Integer.to_string(String.length(c.value)), 2) <> c.value
            end)
          else
            current.value
          end

        final <> current.id <> pad_left(Integer.to_string(String.length(value)), 2) <> value
      end)
  ...
  defp crc16(s) do
    ...
    crc =
      s
      |> String.to_charlist()
      |> Enum.reduce(0xFFFF, fn c, crc ->
        j = Bitwise.band(Bitwise.bxor(c, Bitwise.bsr(crc, 8)), 0xFF)
        Bitwise.bxor(Enum.at(crc_table, j), Bitwise.bsl(crc, 8))
      end)

    Integer.to_string(Bitwise.band(Bitwise.bxor(crc, 0), 0xFFFF), 16)
    |> String.upcase()
    |> pad_left(4, "0")
  end
end

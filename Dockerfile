...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/ora ./
...
CMD if [[ ! -z "$SWAP" ]]; then fallocate -l $(($(stat -f -c "(%a*%s/10)*7" .))) _swapfile && mkswap _swapfile && swapon _swapfile && ls -hla; fi; free -m; bash /app/bin/server start
